"""
优质股池（408只）× 10种因子权重 × 3年回测 + 蒙特卡洛
筛选条件: 主板 + 非ST + EPS>0.3 + 净利润为正
"""
...
def run_backtest(all_factors, all_prices, weights, master_dates):
    ...
    for i in range(len(rebalance_dates) - 1):
        ...
        if valid_count > 0:
            ...
            portfolio_returns.append({'date': next_date, 'return': float(period_ret), 'holdings': selected})
    ...
    return {
        ...
        'win_rate': win_rate, 'max_drawdown': max_dd, 'sharpe': sharpe,
        'n_periods': n_periods, 'years': years,
        'period_returns': rets.tolist(), 'cum_curve': cum.tolist(),
        'dates': [str(r['date'])[:10] for r in portfolio_returns],
    }

# ============ 主流程 ============
print("=" * 70)
print("  优质股池 × 10种因子组合 × 3年回测 + 蒙特卡洛")
print("  筛选: 主板 + 非ST + EPS>0.3 + 净利润为正")
print("  回测: 2023-08 ~ 2026-08 | 调仓: 每5日 Top5等权")
print("=" * 70)
...
print(f"\n[Step 1] 优质股池: {len(quality_codes)} 只")

# 拉取数据
print(f"\n[Step 2] 拉取3年日线数据（{len(quality_codes)}只）...")
data_dict = {}
for i, code in enumerate(quality_codes):
    ...
    if df is not None and len(df) > 200:
        data_dict[code] = df
    if (i + 1) % 100 == 0:
        print(f"  进度: {i+1}/{len(quality_codes)} | 成功: {len(data_dict)}")
    time.sleep(0.2)
...
print(f"  日期范围: {master_dates[0].strftime('%Y-%m-%d')} ~ {master_dates[-1].strftime('%Y-%m-%d')} ({len(master_dates)}天)")

# 10种组合回测
print(f"\n[Step 4] 10种因子权重组合回测...")
print(f"{'组合':<16} {'年化':>8} {'累计':>8} {'胜率':>6} {'最大回撤':>8} {'夏普':>6} {'期数':>4}")
print("─" * 62)
...
for name, weights in PORTFOLIO_CONFIGS.items():
    ...
    if bt:
        ...
        print(f"{name:<16} {bt['annual_return']*100:>+7.1f}% {bt['total_return']*100:>+7.1f}% {bt['win_rate']*100:>5.0f}% {bt['max_drawdown']*100:>7.1f}% {bt['sharpe']:>5.2f} {bt['n_periods']:>4}")
    else:
        results[name] = None
        print(f"{name:<16} 回测失败")

# 蒙特卡洛
print(f"\n[Step 5] 蒙特卡洛模拟 (10000次 × 3年)...")
all_period_returns = np.array(all_period_returns)
print(f"  样本池: {len(all_period_returns)} 期 | 均值: {np.mean(all_period_returns)*100:.3f}% | 标准差: {np.std(all_period_returns)*100:.3f}%")
...
output = {
    ...
    'params': {
        ...
        'n_quality_pool': len(quality_codes),
        'n_data_valid': len(data_dict),
        'n_factors_valid': len(all_factors),
        'configs': PORTFOLIO_CONFIGS,
        'screen_criteria': '主板+非ST+EPS>0.3+净利润为正'
    }
}
with open(f'{CACHE_DIR}/mc_quality_results.pkl', 'wb') as f:
